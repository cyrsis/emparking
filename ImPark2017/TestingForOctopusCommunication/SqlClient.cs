...
namespace TestingForOctopusCommunication
{
    public class SqlClient
    {
        ...
        public void SucessfulTransaactionUpdateMisc(string deviceID2, string cardID, int balance, int PollStatus, DateTime transDatetime, String ReceiptAddmessage)
        //Update SQL with Checking on balance
        {
            try
            {
                ...
                using (var command = sqlConnection.CreateCommand())
                {
                    command.CommandTimeout = 80;
                    command.CommandType = CommandType.Text;
                    command.CommandText = "UPDATE MISC_TRANS_OCTOPUS SET DEVICE_ID= '" + deviceID2 + "', OCTOPUS_CARD_NO='" + cardID +
                        "', REMAIN_VALUE=" + (Convert.ToDecimal(balance) / 10).ToString("#,##.0") + ", ADD_VALUE_MSG='" + ReceiptAddmessage +
                        "', TRANS_DATE_TIME='" + transDatetime.ToString("yyyyMMddHHmmss") + "',TRANS_NO='12345', STATUS_ID = 1 WHERE STATUS_ID = 2";
                    Form1.log.Info("SQL Statment " + command.CommandText);
                    command.ExecuteNonQuery();
            ...
            try
            {
                        ...
                        ConfigurationManager.ConnectionStrings["Carpark_ClientConnection"].ConnectionString);
                {
                    ...
                    using (var command = sqlConnection.CreateCommand())
                    {
                        ...
                        command.CommandType = CommandType.Text;
                        command.CommandText =
                            "UPDATE MISC_TRANS_OCTOPUS SET STATUS_ID= 2 WHERE STATUS_ID= 0";
                        Form1.log.Info("SQL Statment " + command.CommandText);
            ...
            try
            {
                        ...
                        ConfigurationManager.ConnectionStrings["Carpark_ClientConnectionMisc"].ConnectionString);
                {
                    ...
                    using (var command = sqlConnection.CreateCommand())
                    {
                        command.CommandTimeout = 55;
                        command.CommandType = CommandType.Text;
                        command.CommandText = "DELETE FROM MISC_TRANS_OCTOPUS WHERE STATUS_ID = 0";
                        Form1.log.Info("SQL Statment " + command.CommandText);
                        ...
                        command.CommandTimeout = 55;
                        command.CommandType = CommandType.Text;
                        command.CommandText = "DELETE FROM MISC_TRANS_OCTOPUS WHERE STATUS_ID = 2";
                        Form1.log.Info("SQL Statment " + command.CommandText);
